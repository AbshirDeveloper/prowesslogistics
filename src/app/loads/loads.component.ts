...
export class LoadsComponent implements OnInit {
  ...
  setForDelete(id, table) {
    this.confirmation = prompt(
      "please enter you password to delete this " + table
    );
  ...
  itExists;
  checkLoadNumber() {
    const load_exist = this.loads.filter(
      el => el.ref_id === this.load["ref_id"]
    );
    load_exist.length > 0
      ? (this.loadNumberError = "This load exists")
      : (this.loadNumberError = null);
  ...
  register_load() {
    if (this.itExists) {
      ...
      if (
        this.load["delivery_time"] &&
        new Date(this.load["delivery_time"]).getTime() >
          new Date(this.load["pick_up_date"]).getTime()
      ) {
  ...
  total;
  calculate() {
    this.total = this.perMile
      ? +this.load["charge"] * +this.load["miles"]
      : +this.load["charge"];
  }

  calculateForEdit() {
    this.total = this.perMile
      ? +this.selected_load["charge"] * +this.selected_load["miles"]
      : +this.selected_load["charge"];
  }
  ...
  check_pickup() {
    if (this.loadTime === "current") {
      new Date(this.load["pick_up_date"]).getTime() <= this.today
        ? (document.getElementById("pick_error").style.display = "block")
  ...
  check_delivery() {
    if (this.loadTime === "current") {
      new Date(this.load["delivery_time"]).getTime() < this.today ||
      new Date(this.load["delivery_time"]).getTime() <
        new Date(this.load["pick_up_date"]).getTime()
        ? (document.getElementById("drop_error").style.display = "block")
        : (document.getElementById("drop_error").style.display = "none");
    } else {
      new Date(this.load["delivery_time"]).getTime() <
      new Date(this.load["pick_up_date"]).getTime()
        ? (document.getElementById("drop_error").style.display = "block")
